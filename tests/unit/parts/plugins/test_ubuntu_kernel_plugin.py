...
def get_kernel_tool_cmds(
    ...
    if kernel_tools:
        ...
        for tool in kernel_tools:
            kernel_tools_cmds.append(
                f"sed -i 's/^\\s*do_tools_{tool}\\s*=.*/do_tools_{tool} = true/g' debian.master/rules.d/{target_arch}.mk"
            )
...
def get_test_fixture_ids() -> list[str]:
    ...
    return [f"{x.base}, {x.arch_build_for}" for x in parms]
